...
class Article(models.Model,ReadNumExpandMethod,LikeNumExpandMethod):
    author = models.ForeignKey(User, on_delete=models.DO_NOTHING,default=None)
    title = models.CharField(max_length=50)
    article_type = models.ForeignKey(ArticleType, on_delete=models.DO_NOTHING,default=None)
    content = RichTextUploadingField()
    created_time = models.DateTimeField(auto_now_add=False)
    last_updated_time = models.DateTimeField(auto_now=True)
    create_month = models.CharField(max_length=50,blank=False,default=calendar.month_name[int(time.localtime().tm_mon)][:3])
    def __str__(self):
